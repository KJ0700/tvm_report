...
# Reverse mapping for quick lookup (all uppercase since we normalize intentName)
INTENT_TO_CATEGORY = {}
...
def create_intent_pie_chart(intent_stats):
    """Create bar chart for individual intents"""
    
    # Sort by count for better visualization
    intent_stats_sorted = intent_stats.sort_values('count', ascending=True)
    
    fig = go.Figure(data=[go.Bar(
        x=intent_stats_sorted['count'],
        y=intent_stats_sorted.index,
        orientation='h',
        text=intent_stats_sorted['percentage'].apply(lambda x: f'{x:.1f}%'),
        textposition='outside',
        marker=dict(
            color=intent_stats_sorted['count'],
            colorscale='Viridis',
    ...
    fig.update_layout(
        ...
        height=max(500, len(intent_stats) * 30),  # Dynamic height based on number of intents
        xaxis_title="Interactions",
        yaxis_title="Intent",
        showlegend=False,
        margin=dict(l=200)  # Left margin for intent names
    )
...
def dealer_selection_page(df, dealer_details_df):
    ...
    st.info(f"📊 Total Available Dealers: **{len(dealer_df)}**")
    ...
    for idx, row in filtered_df.iterrows():
        ...
        with col1:
            checked = st.checkbox("Select dealer", value=is_selected, key=f"cb_{dealer_id}_v{st.session_state.selection_version}", label_visibility="collapsed")
    ...
    if st.session_state.selected_dealers:
        ...
        for dealer_id in st.session_state.selected_dealers:
            ...
            if not dealer_info.empty:
                ...
                selected_dealer_info.append(f"• **{client_name}** (`{dealer_id}`)")
    ...
    with col1:
        if st.button("📊 Analyze Selected", type="primary", width='stretch', disabled=len(st.session_state.selected_dealers) == 0):
            st.session_state.page = 'analysis'
            st.rerun()
    
    with col2:
        if st.button("📈 Analyze All Dealers", width='stretch'):
...
def main():
    ...
    try:
        ...
        try:
            ...
            st.sidebar.warning("⚠️ ProdDealerDetails.csv not found. Analyzing all dealers.")
    ...
    if dealer_details_available:
        if st.sidebar.button("🏢 Dealer Selection", width='stretch', type="primary" if st.session_state.page == 'selection' else "secondary"):
            st.session_state.page = 'selection'
            st.rerun()
        
        if st.sidebar.button("📊 Analysis Dashboard", width='stretch', disabled=len(st.session_state.selected_dealers) == 0, type="primary" if st.session_state.page == 'analysis' else "secondary"):
            st.session_state.page = 'analysis'
            st.rerun()
    ...
    if dealer_details_available and st.session_state.page == 'selection':
        ...
        if st.session_state.selected_dealers:
            st.sidebar.success(f"✅ {len(st.session_state.selected_dealers)} dealer(s) selected")
        return
    ...
    if dealer_details_available and st.session_state.selected_dealers:
        ...
        st.info(f"📊 Analyzing {len(st.session_state.selected_dealers)} selected dealer(s)")
        
        # Display selected dealers with their domains
        with st.expander("🏢 View Selected Dealers & Domains", expanded=False):
            dealers_info = []
    ...
    if BERTOPIC_AVAILABLE and show_topic_modeling:
        ...
        tab1, tab2, tab3, tab4 = st.tabs([
            f"📊 {get_intent_display_name('GSL')}", 
            f"📊 {get_intent_display_name('GSR')}", 
            f"🚫 {get_intent_display_name('IR')}", 
            f"❓ {get_intent_display_name('FAQ')}"
        ])
